...
b <- X02092020_cyto_alldata_usedfor_plots
q <- ggplot(data=b, aes(x=CHX_NOER_Cyto_vs_CHX_ER_Cyto.log2fc, y=-log10((CHX_NOER_Cyto_vs_CHX_ER_Cyto.worst_fdr)))) + geom_point()+theme_minimal()
...
b$diffexpressed[b$CHX_NOER_Cyto_vs_CHX_ER_Cyto.log2fc > 0.85 & b$CHX_NOER_Cyto_vs_CHX_ER_Cyto.worst_fdr < 0.05] <- "UP"
# if log2Foldchange < -0.85 and pvalue < 0.05, set as "DOWN"
b$diffexpressed[b$CHX_NOER_Cyto_vs_CHX_ER_Cyto.log2fc < -0.85 & b$CHX_NOER_Cyto_vs_CHX_ER_Cyto.worst_fdr < 0.05] <- "DOWN"


q <- ggplot(data=b, aes(x=CHX_NOER_Cyto_vs_CHX_ER_Cyto.log2fc, y=-log10(CHX_NOER_Cyto_vs_CHX_ER_Cyto.worst_fdr), col=diffexpressed)) + 
...
ggsave("CHX_NOER vs CHX_ER Cytoplasm_biotypes.png", 
